...
def gd_loss(theta, state, path, slack, t_param):
    total_cost, state, steps_cost, t = total_cost_calc(state, theta, slack, path, theta.shape[1], t_param, new_state=True)
    return total_cost
...
def gd_step(theta, slack, control_dist, learn_rate, state=None, path=None, t_param=0):
    ...
    for i in range(10):
        ...
        g_t += (theta - u) * gd_loss(u, state, path, slack, t_param) / sigma_sqr_gd
